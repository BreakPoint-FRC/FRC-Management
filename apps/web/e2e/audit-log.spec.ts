...
test("paginates, filters, validates dates and clears back to page one", async ({ page }) => {
  ...
  await expect(page.locator("tbody tr")).toHaveCount(25);
  await page.getByLabel("Kayıt türü").selectOption("GROUP");
  await page.getByLabel("Başlangıç").fill("2026-09-07");
  await page.getByLabel("Bitiş").fill("2026-09-06");
  await page.getByRole("button", { name: "Uygula" }).click();
  expect(await page.getByLabel("Başlangıç").evaluate((input: HTMLInputElement) => input.validity.valid)).toBe(false);
  expect(queries.at(-1)!.searchParams.has("entityType")).toBe(false);
  await page.getByLabel("Başlangıç").fill("2026-09-01");
  await page.getByRole("button", { name: "Uygula" }).click();
  await expect(page.getByText("Filtrelere uygun denetim kaydı yok.")).toBeVisible();
...
for (const failure of ["500", "network", "empty", "403"] as const) {
  test(`renders ${failure} and retries recoverable failures`, async ({ page }) => {
    ...
    if (failure === "403") await expect(page.locator("main [role=alert]")).toHaveText("Denetim kaydını görüntüleme yetkiniz yok.");
      ...
      await expect(page.getByText(failure === "network" ? "İnternet bağlantısı yok" : "Test server error", { exact: true })).toBeVisible();
      recovered = true;
...
for (const width of [360, 1280]) for (const colorScheme of ["light", "dark"] as const) {
  test(`layout ${width}px ${colorScheme}`, async ({ page }, testInfo) => {
    ...
    await expect(change).toContainText("Güncelleme");
