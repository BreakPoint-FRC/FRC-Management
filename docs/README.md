# BreakPoint docs

- [migrations.md](migrations.md) — how to change the database safely
- [documentation.md](documentation.md) — what has to be documented, and where
- [product/](product/) — what BreakPoint is for and what ships when
  - [problems.md](product/problems.md) — the team problems this solves
  - [roadmap.md](product/roadmap.md) — V1 / V2 / V3 scope
  - [stack.md](product/stack.md) — the stack decision
  - [meeting-notes.md](product/meeting-notes.md) — decisions taken in meetings

Start with the [root README](../README.md) to get the app running, and
[CONTRIBUTING.md](../CONTRIBUTING.md) before your first pull request.
