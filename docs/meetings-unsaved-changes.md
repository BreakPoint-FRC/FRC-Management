...
The root provider coordinates an application-owned dialog for the editor's
Cancel button, opening another editor/new meeting, meeting detail links, sidebar
links, sign-out, and browser Back/Forward navigation. Cancelling preserves the
draft and current location; confirming discards it and executes the requested
action once. Escape stays in the editor, focus starts on the safe option, Tab
stays within the dialog, and closing the dialog restores focus to its trigger
when that trigger still exists.

Back/Forward entries are indexed without inserting a duplicate history entry.
When a dirty traversal is requested, the provider first restores the current
entry, asks for confirmation, and only replays the original traversal after a
discard decision. Staying, saving, or restoring every original value therefore
does not leave a ghost Back step or truncate an existing Forward stack. The
provider preserves Next.js's own `history.state` fields while adding its marker;
the Back/Forward E2E tests must be rerun when Next.js is upgraded.

Saving disables the editor and blocks these departure actions until the request
finishes. If the user tries to leave while a save is pending, a visible live
status explains why navigation was blocked. Delete buttons are disabled while
an editor is open so its backing record cannot disappear during editing.
Filtering the list and new-tab gestures do not discard the current draft and do
not prompt.

Future application controls that leave this editor must use `GuardedLink` or
`requestLeave`. Forms opt in by registering a guard; the root provider also
coordinates browser history while such a guard is dirty. Forced authentication
redirects, attendance editing, other forms, autosave and persistent draft
...
| Application | Cancel, edit/new meeting, detail/sidebar navigation, sign-out and Back/Forward show the custom dialog, with no native confirm; staying preserves the draft; leaving executes the intended action; reopening a saved form and cancelling does not warn. |

Additional tests cover every field, reverting changes, pending/failed saves,
dirty create mode, ghost-free history, keyboard focus, Escape, real new-tab
gestures, filtering, and mobile light/dark layout.
